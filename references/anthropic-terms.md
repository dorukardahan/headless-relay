...
| xAI (Grok) | Enterprise ToS governs the API. | AUP (effective June 26, 2026) prohibits "Using the Service or any Output to develop machine learning models or related AI services that compete with xAI" and "Scraping, harvesting or reselling any Input or Output, or distilling model data or Outputs"; Enterprise ToS (May 12, 2026) additionally bans using Output to train customer ML/AI models. | Longstanding clauses, tightened through 2026. **Data ingestion (July 2026):** the Grok Build CLI, run inside a git repo, uploads the ENTIRE tracked repo as a git bundle (full history + all tracked files, incl. a tracked `.env`) to xAI GCS `grok-code-session-traces`, independent of files read and not stopped by `--disable-web-search`/deny/toggle (confirmed by xAI's Grok account on X + mitmproxy capture). Consumer opt-out is retention-only (`/privacy`); ZDR is Team/Enterprise-only. This is a data-EXFIL risk to the caller, distinct from the competing-model clauses above — see the isolation rule in [cli-reference.md](cli-reference.md) and [../SECURITY.md](../SECURITY.md). |
...
| Moonshot AI (Kimi) | Kimi Code explicitly supports Kimi CLI, OpenCode, OpenClaw, and other agent frameworks, but subscription benefits are limited to personal interactive use. Native `kimi login` OAuth belongs only to the first-party Kimi CLI; OpenCode's Kimi provider uses a separate API key. | No sharp anti-competing or distillation ban surfaced in the Kimi Code product guidelines reviewed 2026-07-19; Kimi K3 is announced as open-source. Do not infer permission beyond the published guidelines. | Conservatively limit headless handoffs to direct user-triggered turns. Unattended scripts/batch/data annotation, account or API resale, reverse-proxy sharing, and client-identity spoofing are prohibited; use the metered Kimi Platform for cron/services. |
...
plan) but its Output cannot feed competing-model work. GLM remains the clearest permissive
always-available target — open-weight AND Hermes Agent is on Z.ai's officially supported tools
list. Kimi K3 is also open-source and works through supported agent frameworks, but its
subscription lane must stay user-triggered rather than cron/batch automation. For a Codex-driven
...
- Kimi Code CLI login / print-mode reference — https://www.kimi.com/code/docs/en/kimi-code-cli/reference/kimi-command
- Kimi Code third-party agent support — https://www.kimi.com/code/docs/en/third-party-tools/other-coding-agents
