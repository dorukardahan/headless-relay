...
| xAI (Grok) | Enterprise ToS governs the API. | AUP (effective June 26, 2026) prohibits "Using the Service or any Output to develop machine learning models or related AI services that compete with xAI" and "Scraping, harvesting or reselling any Input or Output, or distilling model data or Outputs"; Enterprise ToS (May 12, 2026) additionally bans using Output to train customer ML/AI models. | Longstanding clauses, tightened through 2026. **Data ingestion (historical, addressed 2026-07-15):** earlier *shipped* Grok Build versions, run inside a git repo, uploaded the ENTIRE tracked repo as a git bundle (full history + all tracked files, incl. a tracked `.env`) to xAI GCS `grok-code-session-traces`, independent of files read (confirmed by xAI's Grok account on X + mitmproxy capture). xAI open-sourced Grok Build on 2026-07-15; a source audit shows that path gone, uploads default off, and local config beating remote. One residual: Grok still reads your `~/.claude` / `~/.cursor` / `~/.codex` config into the model turn by default (a compat scan the relay blocks by running grok under a hermetic `env -i` child with an empty `HOME` + a clean temp `GROK_HOME`). A data-EXFIL concern to the caller, distinct from the competing-model clauses above — see [../SECURITY.md](../SECURITY.md). |
