...
relayauth_profile() (   # $1 = auth.json path, $2 = the temp GROK_HOME to write the profile into
  ...
  [ -d "${2:-}" ] || exit 1
  case "$1" in /*) ;; *) exit 1 ;; esac       # absolute only: absolutising a relative path via $(pwd) can strip a newline from the CWD name and silently name a sibling
...
if relayauth_profile "$ap" "$gh"; then
  ( cd "$iso" && env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin HOME="$gh" GROK_HOME="$gh" TMPDIR="$gh" TERM=dumb \
      ...
      "$grokbin" -p "…" -m grok-4.7 --disable-web-search --sandbox relayauth --deny '*' )
else
...
Headless via `-p`. Use `-m grok-4.7` — xAI's coding/agents frontier model (launched
2026-09-21; 500K context; same published $2/$6 per 1M tokens class as 4.6).
Pass `-m` explicitly: defaults drift. `grok-4.6` (2026-08-12) remains served as the previous
...
( cd "$iso" && env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin HOME="$gh" GROK_HOME="$gh" TMPDIR="$gh" TERM=dumb \
    GROK_TELEMETRY_ENABLED=false GROK_TELEMETRY_TRACE_UPLOAD=false GROK_EXTERNAL_OTEL=false GROK_AUTH_PATH="$ap" \
    RUST_LOG=debug "$grokbin" -p "test" -m grok-4.7 --disable-web-search --sandbox relayauth --deny '*' 2>/tmp/grok-debug.log ) &
...
4. "You are not authenticated." with NO model list but auth.json exists → the refresh itself
   ...
   ( cd "$iso" && env -i PATH=/usr/bin:/bin:/usr/sbin:/sbin HOME="$gh" GROK_HOME="$gh" TMPDIR="$gh" TERM=dumb \
       ...
       "$grokbin" -p "Reply with exactly GROK_OK and nothing else." -m grok-4.7 --disable-web-search --sandbox relayauth --deny '*' ); rc=$?
   rm -rf "$gh" "$iso"
   ```
...
| Grok: `Couldn't set model 'grok-build': Invalid params: "unknown model id"` | `grok-build` retired from the CLI at the grok-4.5 launch (2026-07-08) | Use a current id, e.g. `-m grok-4.7` |
...
| Grok answer seems shallow | A lighter model (e.g. `grok-composer-2.5-fast`) was selected | Pass `-m grok-4.7` explicitly |
